...
struct Sample {
    ...
    int32_t  rx_ns;         // drain+parse+slot. not parse alone; ekf_ns/ctrl_ns land beside this.
    uint32_t seq;
    uint32_t flags;
    ...
    int64_t  age_ns;        // woke - rx_mono_ns at read. kAgeNone if the slot was never filled.
};
...
enum : uint32_t {
    ...
    FLAG_STALE      = 1u << 3,  // age > expected_period * limit. instrumentation; detector is step 6
};
...
class RingLog {
  public:
    ...
    bool write_csv(const std::string& path, const std::vector<std::string>& meta) const {
        ...
        std::fprintf(f, "seq,deadline_ns,wake_err_ns,exec_ns,rx_ns,flags,rx_count,seq_gaps,skew_ns,age_ns\n");
        for (size_t i = 0; i < n_; ++i) {
            const Sample& s = buf_[i];
            std::fprintf(f, "%u,%lld,%d,%d,%d,%u,%u,%u,%d,%lld\n", s.seq,
                         static_cast<long long>(s.deadline_ns), s.wake_err_ns,
                         s.exec_ns, s.rx_ns, s.flags, static_cast<unsigned>(s.rx_count),
                         static_cast<unsigned>(s.seq_gaps), s.skew_ns,
                         static_cast<long long>(s.age_ns));
        }
        std::fclose(f);
        return true;
    }
